...
struct OptionMenu
{
    ...
    u8 menuCursor[MENU_COUNT];
    u8 visibleCursor[MENU_COUNT];
    u8 arrowTaskId;
};
...
static bool8 CheckConditions(int selection)
{
    ...
    {
    case MENU_MAIN:
        return selection < MENUITEM_MAIN_COUNT;
    case MENU_CUSTOM:
        return selection < MENUITEM_CUSTOM_COUNT;
    default:
        return FALSE;
    }
}
...
static u8 ValidateSubmenu(u8 submenu)
{
    return (submenu < MENU_COUNT) ? submenu : MENU_MAIN;
}
...
static const u8 *const OptionTextDescription(void)
{
    ...
    u8 submenu = ValidateSubmenu(sOptions->submenu);
    ...
    case MENU_MAIN:
        ...
        if (!CheckConditions(menuItem))
            return (menuItem < MENUITEM_MAIN_COUNT) ? sOptionMenuItemDescriptionsDisabledMain[menuItem] : sText_Empty;
            
        selection = sOptions->sel[menuItem];
        if (menuItem == MENUITEM_MAIN_TEXTSPEED || menuItem == MENUITEM_MAIN_FRAMETYPE)
            selection = 0;
            
        return (selection < 3) ? sOptionMenuItemDescriptionsMain[menuItem][selection] : sText_Empty;
    ...
    case MENU_CUSTOM:
        ...
        if (!CheckConditions(menuItem))
            return (menuItem < MENUITEM_CUSTOM_COUNT) ? sOptionMenuItemDescriptionsDisabledCustom[menuItem] : sText_Empty;
            
        selection = sOptions->sel_custom[menuItem];
        if (menuItem == MENUITEM_CUSTOM_HP_BAR || menuItem == MENUITEM_CUSTOM_EXP_BAR)
            selection = 0;
            
        return (selection < 2) ? sOptionMenuItemDescriptionsCustom[menuItem][selection] : sText_Empty;
...
static u8 MenuItemCount(void)
{
    if (sOptions == NULL) return 0;
    
    switch (ValidateSubmenu(sOptions->submenu)) {
...
static void HighlightOptionMenuItem(void)
{
    ...
    u8 submenu = ValidateSubmenu(sOptions->submenu);
    int cursor = sOptions->visibleCursor[submenu];
...
static void Task_OptionMenuProcessInput(u8 taskId)
{
    ...
    else if (JOY_NEW(DPAD_DOWN))
    {
        ...
        else
        {
            ...
            {
                u8 submenu = ValidateSubmenu(sOptions->submenu);
                sOptions->visibleCursor[submenu] = optionsToDraw - 2;
                sOptions->menuCursor[submenu] = MenuItemCount() - optionsToDraw - 1;
                ScrollAll(1);
